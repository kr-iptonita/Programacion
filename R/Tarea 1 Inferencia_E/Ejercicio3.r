...
cat("El coeficiente de asimetría del ingreso corriente es:", coef_asimetria, "\n")
...
coef_asimetria_log <- skewness(datos_filtrados$log_ing_cor, na.rm = TRUE)

# Imprimir el coeficiente de asimetría
cat("El coeficiente de asimetría del logaritmo natural del ingreso corriente es:", coef_asimetria_log, "\n")
...
if(coef_asimetria_log == 0) {
  ...
  cat("La distribución del logaritmo natural del ingreso es sesgada hacia la derecha (positiva).\n")
} else {
  cat("La distribución del logaritmo natural del ingreso es sesgada hacia la izquierda (negativa).\n")
}
...
curtosis_log_ingreso <- kurtosis(datos_filtrados$log_ing_cor, na.rm = TRUE)

# Imprimir la curtosis
cat("La curtosis del logaritmo del ingreso es:", curtosis_log_ingreso, "\n")
...
if(curtosis_log_ingreso == 0) {
  ...
  cat("La forma de la variable presenta una mayor concentración de datos en torno a la media (pico) y colas más pesadas que una distribución normal.\n")
} else {
  cat("La forma de la variable presenta una menor concentración de datos en torno a la media (pico) y colas más ligeras que una distribución normal.\n")
}
...
desviacion_log_ingreso <- sd(datos_filtrados$log_ing_cor, na.rm = TRUE)

# Crear una secuencia de valores para la curva de densidad normal
x <- seq(min(datos_filtrados$log_ing_cor, na.rm = TRUE), max(datos_filtrados$log_ing_cor, na.rm = TRUE), length.out = 100)
# Calcular la densidad normal
densidad_normal <- dnorm(x, mean = media_log_ingreso, sd = desviacion_log_ingreso)

# Graficar densidad estimada del logaritmo del ingreso
plot(density(datos_filtrados$log_ing_cor), main = "Densidad Estimada vs Densidad Normal",
     xlab = "Logaritmo del Ingreso", ylab = "Densidad", ylim = c(0, max(density(datos_filtrados$log_ing_cor)$y, densidad_normal)),
     col = "blue", lwd = 2, xlim = range(x))
# Agregar densidad normal
lines(x, densidad_normal, col = "red", lwd = 2)

legend("topright", legend = c("Densidad Estimada", "Densidad Normal"), col = c("blue", "red"), lty = 1, lwd = 2)
...
cat("El coeficiente de asimetría de la edad de los jefes es:", coef_asimetria, "\n")
...
# Calcular el coeficiente de asimetría de la nueva variable
coef_asimetria_log <- skewness(datos_filtrados$log_edad_jefe, na.rm = TRUE)

# Imprimir el resultado
cat("El coeficiente de asimetría del logaritmo natural de la edad de los jefes es:", coef_asimetria_log, "\n")

# Determinar si la distribución es simétrica o no
if(coef_asimetria_log == 0) {
  cat("La distribución del logaritmo natural de la edad de los jefes es simétrica.\n")
} else if(coef_asimetria_log > 0) {
  cat("La distribución del logaritmo natural de la edad de los jefes es sesgada hacia la derecha (positiva).\n")
} else {
  cat("La distribución del logaritmo natural de la edad de los jefes es sesgada hacia la izquierda (negativa).\n")
}

# Calcular la curtosis del logaritmo del edad de los jefes
curtosis_log_edad_jefe <- kurtosis(datos_filtrados$log_edad_jefe, na.rm = TRUE)

# Imprimir el resultado
cat("La curtosis del logaritmo del edad de los jefes es:", curtosis_log_edad_jefe, "\n")
...
if(curtosis_log_edad_jefe == 0) {
  ...
  cat("La forma del logaritmo del edad de los jefes es puntiaguda (leptocúrtica) en comparación con una distribución normal.\n")
} else {
  cat("La forma del logaritmo del edad de los jefes es achatada (platicúrtica) en comparación con una distribución normal.\n")
}

# Calcular la media y desviación estándar del logaritmo del edad de los jefes
media_log_edad_jefe <- mean(datos_filtrados$log_edad_jefe, na.rm = TRUE)
desviacion_log_edad_jefe <- sd(datos_filtrados$log_edad_jefe, na.rm = TRUE)

# Crear una secuencia de valores para la densidad normal
x <- seq(min(datos_filtrados$log_edad_jefe), max(datos_filtrados$log_edad_jefe), length.out = 100)
densidad_normal <- dnorm(x, mean = media_log_edad_jefe, sd = desviacion_log_edad_jefe)

# Graficar la densidad estimada del logaritmo del edad de los jefes
plot(density(datos_filtrados$log_edad_jefe), main = "Densidad Estimada del Logaritmo del Edad de los Jefes",
     xlab = "Logaritmo del Edad de los Jefes", ylab = "Densidad", xlim = c(min(x), max(x)), ylim = c(0, max(density(datos_filtrados$log_edad_jefe)$y, densidad_normal)),
     col = "blue", lwd = 2)
...
legend("topright", legend = c("Densidad Estimada", "Densidad Normal"), col = c("blue", "red"), lwd = 2)

##3F##

# Calcular la covarianza entre la edad del jefe del hogar y el ingreso corriente
cov_edad_ingreso <- cov(datos_filtrados$edad_jefe, datos_filtrados$ing_cor, use = "complete.obs")

# Calcular la correlación entre la edad del jefe del hogar y el ingreso corriente
cor_edad_ingreso <- cor(datos_filtrados$edad_jefe, datos_filtrados$ing_cor, use = "complete.obs")

# Imprimir los resultados
cat("La covarianza entre la edad del jefe del hogar y el ingreso corriente es:", cov_edad_ingreso, "\n")
cat("La correlación entre la edad del jefe del hogar y el ingreso corriente es:", cor_edad_ingreso, "\n")

# Determinar si parece haber alguna relación lineal entre las dos variables
if(abs(cor_edad_ingreso) >= 0.7) {
  cat("Sí parece haber una relación lineal fuerte entre la edad del jefe del hogar y el ingreso corriente.\n")
} else if(abs(cor_edad_ingreso) >= 0.3) {
  cat("Sí parece haber una relación lineal moderada entre la edad del jefe del hogar y el ingreso corriente.\n")
} else if(abs(cor_edad_ingreso) > 0) {
  cat("Sí parece haber una relación lineal débil entre la edad del jefe del hogar y el ingreso corriente.\n")
} else {
  cat("No parece haber una relación lineal entre la edad del jefe del hogar y el ingreso corriente.\n")
}