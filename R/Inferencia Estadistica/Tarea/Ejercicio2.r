...
datos_filtrados <-
  datos[datos$ubica_geo >= 10000 & datos$ubica_geo <= 11999,]
...
# Resultados
cat("El promedio de la cantidad de integrantes por casa es:",
    promedio_tot_integ,
    "\n")
cat("La mediana de la cantidad de integrantes por casa es:",
    mediana_tot_integ,
    "\n")
cat("Son distintos")
...
cat("El promedio de la edad de los jefes del hogar es:",
    promedio_edad_jefe,
    "\n")
cat("La mediana de la edad de los jefes del hogar es:",
    mediana_edad_jefe,
    "\n")
...
prop_con_educacion <-
  mean(datos_filtrados$educa_jefe >= 6, na.rm = TRUE)
...
# Imprimir los resultados
cat(
  "La proporción de jefes de hogar con educación básica completa es:",
  prop_con_educacion,
  "\n"
)
cat(
  "La proporción de jefes de hogar sin educación básica completa es:",
  prop_sin_educacion,
  "\n"
)
...
promedio_ingreso_completo <-
  mean(datos_filtrados$ing_cor[datos_filtrados$educa_jefe >= 6], na.rm = TRUE)

# Calcular el promedio del ingreso corriente en los hogares con educación básica incompleta (educa_jefe < 6)
promedio_ingreso_incompleto <-
  mean(datos_filtrados$ing_cor[datos_filtrados$educa_jefe < 6], na.rm = TRUE)

# Imprimir los resultados
cat(
  "El promedio del ingreso corriente en hogares con educación básica completa es:",
  promedio_ingreso_completo,
  "\n"
)
cat(
  "El promedio del ingreso corriente en hogares con educación básica incompleta es:",
  promedio_ingreso_incompleto,
  "\n"
)
...
if (promedio_ingreso_completo > promedio_ingreso_incompleto) {
  ...
  cat(
    "El promedio del ingreso corriente es mayor en hogares con educación básica incompleta.\n"
  )
} else {
  cat(
    "No parece haber diferencia en los ingresos entre hogares con educación básica completa e incompleta.\n"
  )
}
...
datos_filtrados$educacion <-
  ifelse(datos_filtrados$educa_jefe >= 6,
         "Básica Completa",
         "Básica Incompleta")
...
legend(
  "topright",
  legend = c("Básica Completa", "Básica Incompleta"),
...
densidad_incompleta <-
  density(datos_filtrados$ing_cor[datos_filtrados$educa_jefe < 6], na.rm = TRUE)
densidad_completa <-
  density(datos_filtrados$ing_cor[datos_filtrados$educa_jefe >= 6], na.rm = TRUE)
...
plot(
  ...
  col = "blue",
  ylim = c(0, max(
    densidad_incompleta$y, densidad_completa$y
  )),
...
legend(
  "topright",
  legend = c("Educación Básica Incompleta", "Educación Básica Completa"),
  col = c("blue", "red"),
  lty = 1
)
...
varianza_incompleta <-
  var(datos_filtrados$ing_cor[datos_filtrados$educa_jefe < 6], na.rm = TRUE)
varianza_completa <-
  var(datos_filtrados$ing_cor[datos_filtrados$educa_jefe >= 6], na.rm = TRUE)

# Imprimir las varianzas
cat(
  "La varianza del ingreso corriente en hogares con educación básica incompleta es:",
  varianza_incompleta,
  "\n"
)
cat(
  "La varianza del ingreso corriente en hogares con educación básica completa es:",
  varianza_completa,
  "\n"
)
...
if (varianza_incompleta > varianza_completa) {
  cat(
    "La varianza del ingreso corriente es mayor en hogares con educación básica incompleta.\n"
  )
  ...
  cat(
    "La varianza del ingreso corriente es igual en hogares con educación básica incompleta y completa.\n"
  )
}